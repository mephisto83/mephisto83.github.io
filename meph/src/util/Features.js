MEPH.define('MEPH.util.Features', {
    ...
    createMatrix: function (width, height, feat) {
        var me = this;
    ...
    createCorners: function (width, height) {
        ...
        var i = width * height;
        while (--i >= 0) {
            corners[i] = new Features.jsfeat.keypoint_t(0, 0, 0, 0);
        }
        return corners;
    },
    setFastCornerThreshold: function (threshold) {
        var me = this;
        var Features = MEPH.util.Features;
    ...
    detectRingCodeCenter: function (canvas, options) {
        ...
        return MEPH.util.Task.taskable(featureResults.corners).task.subset(0, featureResults.count)
            .then(function (corners) {
                return corners.task.select(function (x) {
            ...
            .then(function (corners) {
                return corners.task.select(function (potentialCog) {
                    var newpos = [].interpSquare(x, y, function ($x, $y) {
                        var x_ = (spread * ($x - (x / 2))) + potentialCog.x;
                        var y_ = (spread * ($y - (y / 2))) + potentialCog.y;
                    ...
                    return temp.task.where()
                        .then(function (temp) {
                            return temp.task.select(function (x) {
                                var starttime = Date.now();
                                var distances = temp.select(function (corner) {
                                ...
                                console.log('Estimation time : ' + (((Date.now() - starttime) / 1000)));
                                return x;
                            }).then(function (temp) {
                                return temp.task.where();
                            }).then(function (temp) {
    ...
    getColorAt: function (x, y, step, imageData) {
        ...
        var temp = new MEPH.math.Vector.Create([r, g, b, 255]);
        return temp;
    },
    detectFeatures: function (canvas, options) {
        ...
        if (!(me.lastWidth === width && me.lastHeight === height)) {
            ...
            me.corners = me.createCorners(width, height);
        }

        me.lastWidth = me.width; me.lastHeight = height;
        if (me.lastThreshold !== (options.threshold || 20)) {
            me.lastThreshold = me.setFastCornerThreshold(options.threshold || 20);
        }

        Features.jsfeat.imgproc.grayscale(imageData.data, width, height, me.matrix);
        var count = Features.jsfeat.fast_corners.detect(me.matrix, me.corners, 5);
        var data_u32 = new Uint32Array(imageData.data.buffer);
        if (options.render) {
            me.render_corners(me.corners, count, data_u32, canvas.width);
    ...
    render_corners: function render_corners(corners, count, img, step) {
    ...
    */
    ...
    MEPH.util.Features.jsfeat = jsfeat;
});