...
Vex.Flow.VexTab.prototype.parseOpenChord = function () {
    ...
    this.parse_state.voices.push(this.state.current_voice);
...
Vex.Flow.VexTab.prototype.parseChordFret = function () {
    ...
    if (isNaN(parseInt(fret))) {
        if (Vex.Flow.keyProperties.note_values[fret]) {
        }
        else
    ...
    this.getNextToken();
    while (Vex.Flow.VexTab.Accidentals[this.parse_state.value] != undefined) {
        fret += Vex.Flow.VexTab.Accidentals[this.parse_state.value];
        this.getNextToken();
    }
...
Vex.Flow.VexTab.prototype.parseFret = function () {
    ...
    if (isNaN(parseInt(str))) {
        if (Vex.Flow.keyProperties.note_values[str]) {
        }
        else
    ...
    var skip = true;
    while (Vex.Flow.VexTab.Accidentals[this.parse_state.value] != undefined) {
        str += Vex.Flow.VexTab.Accidentals[this.parse_state.value];
        this.getNextToken();
        skip = false;
    }
...
Vex.Flow.VexTab.prototype.genElements = function () {
    ...
    var voices = this.parse_state.voices;
    ...
    for (var i = 0; i < positions.length; ++i) {
        ...
        var tabnote = new Vex.Flow.TabNote(
            { positions: position, duration: duration, voices: voice });
        ...
        if (this.state.has_notation) {
            ...
            for (var j = 0; j < position.length; ++j) {
                ...
                if (isNaN(notefret.fret))
                    var spec = notefret.fret + "/" + notefret.str;
                else
                    var spec = this.tuning.getNoteForFret(notefret.fret, notefret.str);
                var spec_props = Vex.Flow.keyProperties(spec);

                var selected_note = this.state.key_manager.selectNote(spec_props.key, spec_props.octave);
                ...
                if (this.music.getNoteParts(selected_note.note.split('/')[0]).root == "b" &&
                    this.music.getNoteParts(spec_props.key).root == "c") {
                    new_octave--;
                }
                else if (this.music.getNoteParts(selected_note.note.split('/')[0]).root == "c" &&
            ...
            var note = new Vex.Flow.StaveNote({
                keys: keys, duration: duration, clef: clef, voices: voice
            });
