...
var midiMaster = (function (window) {
    ...
    MidiEvent.createNoteFromCC = function (note, step) {
        if (!note) { throw new Error("Note not specified"); }

        var events = [];
        var t = "0x" + note[0].event.noteNumber.toString(16);
        var noteevent = note[0].event;
        if (note[0].event.subtype == "noteOn") {
            //channel: 0
            //deltaTime: 0
            //noteNumber: 60
            //subtype: "noteOn"
            //type: "channel"
            //velocity: 110
            events.push(MidiEvent.noteOn({ duration: step, time: note[1], pitch: t, volume: noteevent.velocity }));
        }
        else {
            events.push(MidiEvent.noteOff({ duration: step, pitch: t, volume: noteevent.velocity }));
        }
        return events;
    };
    /**
